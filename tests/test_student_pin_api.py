...
def test_get_classmates(client, class_payload, teacher_payload):
    """
    Integration test to verify classmates can view each other via /class/{class_id}/members.
    - A teacher creates a class.
    - Two students register and join the class.
    - One student fetches classmates.
    - Validates that both students are returned in the member list.
    """
    ...
    create_class = client.post("/class/create", json=unique_class, headers=teacher_headers)
    assert create_class.status_code == 201
    
    
    class_id = create_class.json()["data"]["id"]
    passphrase = create_class.json()["data"]["passphrase"]

    student1_payload = {
        "user_id": f"student1_{uuid.uuid4().hex[:6]}",
        "first_name": "Alice",
        "last_name": "Anderson",
        "password": "MyCoolPassword##",
        "user_type": "student"
    ...
    student2_payload = {
        "user_id": f"student2_{uuid.uuid4().hex[:6]}",
        "first_name": "Bob",
        "last_name": "Brown",
        "password": "MyCoolPassword##",
        "user_type": "student"
    ...
    for student in (student1_payload, student2_payload):
        reg_resp = client.post("/user/register", json=student)
        assert reg_resp.status_code == 201

    login1 = client.post("/user/login", data={
        "username": student1_payload["user_id"],
        "password": student1_payload["password"]
    })
    student1_token = login1.json()["data"]["access_token"]
    student1_headers = {"Authorization": f"Bearer {student1_token}"}

    login2 = client.post("/user/login", data={
        "username": student2_payload["user_id"],
        "password": student2_payload["password"]
    })
    student2_token = login2.json()["data"]["access_token"]
    student2_headers = {"Authorization": f"Bearer {student2_token}"}

    # Step 4: Both students join the class
    for headers in (student1_headers, student2_headers):
        join_resp = client.post("/class/join", json={"passphrase": passphrase}, headers=headers)
        assert join_resp.status_code == 200

    # Step 5: Student1 fetches classmates
    members_resp = client.get(f"/class/{class_id}/members", headers=student1_headers)
    assert members_resp.status_code == 200
    resp_data = members_resp.json()["data"]
    assert isinstance(resp_data, list)
    assert len(resp_data) >= 2  # At least the 2 students

    user_ids = {student["user_id"] for student in resp_data}
    assert student1_payload["user_id"] in user_ids
    assert student2_payload["user_id"] in user_ids

    # Optional: Check sorting
    members_resp_sorted = client.get(
        f"/class/{class_id}/members?sort_by=first_name&order=desc",
        headers=student1_headers
    )
    assert members_resp_sorted.status_code == 200
    sorted_data = members_resp_sorted.json()["data"]
    first_names = [m["first_name"] for m in sorted_data]
    assert first_names == sorted(first_names, reverse=True)


def test_rename_class(client, teacher_payload, class_payload):
    """
    Integration test for renaming a class:
    - Registers and logs in a teacher.
    - Creates a class.
    - Renames the class using the /class/{class_id}/rename endpoint.
    - Verifies successful rename and that the new name is persisted.
    """
    ...
    new_name = f"{class_payload['name']}_Renamed"
    rename_payload = {"name": new_name}
    rename_resp = client.patch(f"/class/{class_id}/rename", json=rename_payload, headers=headers)

    assert rename_resp.status_code == 200

    resp_json = rename_resp.json()

    assert resp_json["success"] is True
    assert resp_json["message"] == "Class renamed successfully"
    assert resp_json["data"]["id"] == class_id
    assert resp_json["data"]["name"] == new_name

    # Optional: Rename again with same name to test idempotency
    idempotent_resp = client.patch(f"/class/{class_id}/rename", json=rename_payload, headers=headers)
    assert idempotent_resp.status_code == 200
    assert idempotent_resp.json()["message"] == "Class name is unchanged"
